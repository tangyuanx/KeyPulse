...
- 内置“检查更新”，可从 GitHub Release 下载新版并自动重启
...
5. 点击“检查更新”会读取本仓库最新 Release；确认后自动下载、替换当前 EXE 并重启。
...
当前版本写在根目录的 `VERSION` 文件中。修改版本号并推送到 `main` 后，GitHub Actions 会构建 Windows EXE；如果对应的 `v版本号` Release 尚不存在，则自动创建 Release 并上传 `KeyPulse.exe`。
