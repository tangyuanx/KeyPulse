...
- 仅在用户点击“检查更新”时查询版本，不在后台自动联网
- 发现新版本后打开对应的 GitHub Release，由用户下载、替换并重新启动
- 通过 Release 的 `VERSION` 文件发现新版本，不消耗 GitHub REST API 的未认证请求额度
- 检查版本时支持 Windows 自动代理、默认代理和直连三种方式重试，GitHub 响应超时为 60 秒
...
3. 关闭窗口会最小化到托盘；右键托盘图标可以打开、导出、检查更新或退出。
4. 点击日期按钮打开日历，可查看或导出历史某天的完整统计。
5. 点击窗口右上角“导出图片”打开分享预览，选择 A1、B 或 C 模板；确认效果后点击“保存图片”并选择保存位置，关闭或取消预览不会生成文件。
6. 点击“检查更新”会读取本仓库最新 Release。发现新版本后，按钮会变为绿色“前往下载”；点击后使用浏览器打开对应 Release。退出 KeyPulse、下载新版并替换旧版后重新启动即可，统计数据不会丢失。
...
KeyPulse 只把 Windows 虚拟键码映射为固定计数器，例如 `A → 502`。程序不会保存字符序列、组合键顺序、窗口标题、进程名称、剪贴板或网络数据。所有统计数据均保存在本机；只有用户主动点击“检查更新”时才会向 GitHub 请求版本文件，不会上传统计数据。
